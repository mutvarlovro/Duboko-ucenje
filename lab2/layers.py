...
class FC(Layer):
  ...
  def forward(self, inputs):
    """
    Args:inputs: ndarray of shape (N, num_inputs)
    Returns:An ndarray of shape (N, num_outputs)
    """
    self.forward_pass_inputs = inputs
    h = np.matmul(inputs, np.transpose(self.weights)) + self.bias # (N, num_outputs)
    return h

  def backward_inputs(self, grads):
    """
    Args:grads: ndarray of shape (N, num_outputs)
    Returns:An ndarray of shape (N, num_inputs)
    """
  ...
  def backward_params(self, grads):
    """
    Args:grads: ndarray of shape (N, num_outputs)
    Returns:List of params and gradient pairs.
    """
    grad_weights = np.matmul(np.transpose(grads), self.forward_pass_inputs)
...
class ReLU(Layer):
  ...
  def forward(self, inputs):
    """
    Args:inputs: ndarray of shape (N, C, H, W).
    Returns:ndarray of shape (N, C, H, W).
    """
    self.forward_pass_inputs = inputs
    inputs[inputs <= 0] = 0
    return inputs
  ...
  def backward_inputs(self, grads):
    """
    Args:grads: ndarray of shape (N, C, H, W).
    Returns:ndarray of shape (N, C, H, W).
    """
    grads[self.forward_pass_inputs <= 0] = 0
    return grads
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    probs = np.sum(np.log(probs) * y, axis=1)
    loss = - np.mean(probs)
    return loss
  ...
  def backward_inputs(self, x, y):
    ...
    Args:
      x: ndarray of shape (N, num_classes).
      y: ndarray of shape (N, num_classes).
    Returns:Gradient with respect to the x, ndarray of shape (N, num_classes).
    """
...
class L2Regularizer():
  ...
  def forward(self):
    """
     Returns:Scalar, loss due to the L2 regularization.
    """
    norm = np.linalg.norm(self.weights)**2
    return 0.5 * self.weight_decay * norm

  def backward_params(self):
    """
    Returns:Gradient of the L2 loss with respect to the regularized weights.
    """
